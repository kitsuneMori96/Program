...
class TqdmHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            if TQDM_AVAILABLE:
                _tqdm.write(msg, file=sys.stderr, end="\n")
            else:
...
class ProgressBar:
    """Async-safe progress bar manager for concurrent downloads.

    Each download acquires a unique terminal row position so multiple
    progress bars can display simultaneously without interfering.
    """

    _bar_format = (
        "{desc:>20.20s} |{bar:20}| {percentage:3.0f}% "
        "{n_fmt:>7s}/{total_fmt:<7s} [{rate_fmt:>10s}] {elapsed:<6s}"
    )

    def __init__(
        self, total: int, desc: str, unit: str = "B", position: int = 1
    ) -> None:
        self._total = total
        self._desc = desc[:20] if len(desc) > 20 else desc
        self._unit = unit
    ...
    def __enter__(self) -> "ProgressBar":
        if TQDM_AVAILABLE and self._total > 0:
            self._bar = _tqdm(
                total=self._total,
                desc=self._desc,
                unit=self._unit,
                unit_scale=self._unit == "B",
                unit_divisor=1024,
    ...
    def write(msg: str) -> None:
        """Write a log message without disrupting progress bars."""
        if TQDM_AVAILABLE:
            _tqdm.write(msg, file=sys.stderr, end="\n")
        else:
            sys.stderr.write(msg + "\n")
...
class ProgressPositionPool:
    """Pool of terminal row positions for concurrent progress bars.

    Position 0 is reserved for an optional overall bar; file-download
    bars use positions 1..max_concurrency.
    """
...
class BaseDownloader:
    ...
    def __init__(
        ...
        self._pool = ProgressPositionPool(config.max_concurrency) if config.show_progress else None
            ...
            try:
                ...
                async with session.get(
                    ...
                    try:
                        ...
                        if self._pool is not None:
                            await self._pool.release(position)
